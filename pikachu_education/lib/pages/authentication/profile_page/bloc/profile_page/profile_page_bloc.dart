...
class ProfilePageBloc extends Bloc<ProfilePageEvent, ProfilePageState> {
  ...
  _updateProfileEvent(UpdateProfileEvent event, Emitter<ProfilePageState> emit) async {
    try{
      emit(UpdateProfileLoadingState());
      await AuthRepositories.updateCurrentUserInfo(itemToUpdate: event.itemToUpdate);
  ...
  _fetchProfilePageData(FetchProfilePageData event, Emitter<ProfilePageState> emit) async {
    try{
  ...
  _refreshProfilePage(RefreshProfilePage event, Emitter<ProfilePageState> emit) async {
    try{
      var currentUserInfo = await AuthRepositories.getCurrentUserInfo(userID: event.userId);
      emit(FetchProfileInfoSuccessState(currentUserInfo: currentUserInfo));
    }catch (e){
